...
OAuth.registerService('dropbox', 2, null, function(query) {
    ...
    // need to remove invalid keys starting with '.' as they cannot be saved in MongoDB.
    var fields = removeInvalidKeysFromObject(identity);
...
var removeInvalidKeysFromObject = function(obj) {
  ...
  for (prop in resultObj) {
    if (prop.startsWith('.')) {
      ...
      if (_.isObject(resultObj[prop])) {
        resultObj[prop] = removeInvalidKeysFromObject(resultObj[prop]);
      }
    }
  }
